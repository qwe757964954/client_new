...
export class MapTouchBetterController extends Component {
    ...
    public moveToTargetPos(targetPos: Vec3): void {
        ...
        if (!visibleRect.contains(v2(relativePos.x, relativePos.y))) {
            ...
            } else if (relativePos.x > visibleRect.x + visibleRect.width) {
                offsetX = relativePos.x - (visibleRect.x + visibleRect.width);
            }
            if (relativePos.y < visibleRect.y) {
                offsetY = relativePos.y - visibleRect.y;
