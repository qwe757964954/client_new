...
export default class CaptureUtils {
    ...
    public static capture(area?: cc.Scene | cc.Node, rect?: IRect): cc.SpriteFrame {
        ...
        if (rect.width === void0) {
            rect.width = Math.abs(info.width * info.scaleX);
        }
        if (rect.height === void0) {
            rect.height = Math.abs(info.height * info.scaleY);
        }
        if (rect.x === void0) {
     ...
     */
    public static readPixels(texture: cc.__private._cocos_asset_assets_texture_base__TextureBase, filpY?: boolean): Uint8Array {
        // 通用版本
     ...
     */
    public static toImgUrl(texture: cc.__private._cocos_asset_assets_texture_base__TextureBase): string {
        ...
        if (cc.sys.isNative) {
            ...
            let filePath = cc.native.fileUtils.getWritablePath() + 'tmpImg.png';/* 临时文件名 */
            cc.native.saveImageData(data, width, height, filePath);
            url = filePath;
        }
        else {
